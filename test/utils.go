...
func SameFile(filename1, filename2 string) (bool, error) {
	f1, err1 := os.ReadFile(filename1)

	if err1 != nil {
		return false, err1
	}

	f2, err2 := os.ReadFile(filename2)
...
func InitSurfServers(blockStores int) []*exec.Cmd {
	...
	if blockStores == 0 {
		serverCmd := exec.Command("_bin/server", "-s", "both", "-l", "localhost:8080")
		...
		for i := 1; i <= blockStores; i++ {
			port := 8080 + i
			blockCmd := exec.Command("_bin/server", "-s", "block", "-p", strconv.Itoa(port), "-l")
		...
		metaCmd := exec.Command("_bin/server", metaArgs...)
...
func KillSurfServers(servers []*exec.Cmd) {
	...
	exec.Command("pkill", "server*")
}

func SyncClient(metaAddr, baseDir string, blockSize int, cfgPath string) error {
	clientCmd := exec.Command("_bin/client", "-d", "-f", cfgPath, baseDir, strconv.Itoa(blockSize))
